...
Our palette is anchored by a sophisticated Amber/Orange (`#F38A2E`), chosen for its high visibility and reduced blue-light strain.
...
- **The Glass & Gradient Rule:** Floating elements (modals, popovers) must use Glassmorphism. Apply `surface` (#121416) at 90% opacity with a `20px` backdrop-blur. 
- **Signature CTA Texture:** Main actions should not be flat. Use a subtle linear gradient from `primary` (#FFB783) to `primary_container` (#F38A2E) at a 135-degree angle to provide a "machined" metallic sheen.

| Token | Value | Role |
| :--- | :--- | :--- |
| `primary` | #FFB783 | Highlights and active states. |
| `primary_container` | #F38A2E | Main brand anchor / Clinical warmth. |
| `surface` | #121416 | **Background:** The primary canvas/floor of the interface. |
| `on_surface` | #F0F0F3 | **Foreground:** Primary high-contrast text and icons. |
| `on_surface_variant` | #E0D7D0 | **Foreground:** Secondary editorial text and captions. |
| `secondary` | #B5C8DF | Analytical data / Neutral tech accents. |

---

## 2.1 Accessibility & Readability (WCAG 2.2)
To ensure the "Clinical Nocturne" is accessible to all users, including those with spectral sensitivity:
- **Minimum Contrast:** All informative text must maintain a minimum contrast ratio of **4.5:1** against its background.
- **Opacity Limits:** Never use opacities below **70%** for body text or important labels (reaches 4.5:1 ratio). Metadata and decorative elements may use 60%.
- **Legacy Classes:** The use of `text-neutral-*`, `text-brand-*`, or `text-ink` is strictly prohibited as they belong to the legacy high-light system and fail contrast tests in this dark environment.
- **Font Weight:** Prefer `medium` (500) or `semibold` (600) for small text (`body-sm`) to improve glyph definition against dark backgrounds.
